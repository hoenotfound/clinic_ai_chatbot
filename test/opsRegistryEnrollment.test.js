  ...
  return async (url, options = {}) => {
    ...
    if (parsed.host === "api.render.com") {
      ...
      if (method === "POST" && parsed.pathname.endsWith("/deploys")) {
        ...
        return response(201, { id: `dep-${serviceId}` });
      }
    }
...
test("automated enrollment configures both secret env vars, deploys, verifies identity, and upserts only secret-free metadata", async () => {
  const calls = [];
...
test("exact client identity mismatch fails before a registry record is created", async () => {
  const calls = [];
  const fetchImpl = makeRenderFetch({
    calls,
  ...
  markPreparedClientDeployment(prepared, { deployId: "dep-client", deployStatus: "live" });
  await deployPreparedRegistryToken({
    prepared,
    result,
    env: BASE_ENV,
