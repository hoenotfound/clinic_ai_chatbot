/**
 * Hardcoded default clinic config — used to seed the `clinic_config` table
 * in Postgres the very first time the app starts against a fresh database
 * (see db/configRepo.js). After that first run, the DB row is the source of
 * truth and this file is no longer read — staff edit the live config from
 * the portal's Settings page instead (Settings -> saved to Postgres ->
 * applied in memory immediately, no restart needed).
 *
 * This file is also the fallback if the DB is ever unreachable at startup,
 * so the bot degrades to sane defaults rather than crashing.
 *
 * Originally filled in from belecoclinic.com (Aug 2026).
 */
