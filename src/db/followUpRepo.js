...
/**
 * Returns the earliest time a currently eligible outbound message can become a
 * follow-up candidate. The service uses this to sleep until real work is due
 * instead of polling Postgres every minute.
 */
async function getNextCandidateDueAt({ delayMinutes, triggerMode, activatedAt }) {
  const result = await pool.query(
    `SELECT MIN(latest.created_at + ($1::integer * interval '1 minute')) AS due_at
     FROM contacts c
     JOIN LATERAL (
    ...
    [delayMinutes, triggerMode, activatedAt]
  );
